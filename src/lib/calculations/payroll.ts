...
/**
 * Calculate daily salary rate from bi-weekly salary
 * Assumes 14-day pay cycle
 */
export function calculateDailySalary(biweeklySalary: number): number {
  return biweeklySalary / 14
}
...
export function calculateLeaveCashout(
  ...
  const dailyRate = calculateDailySalary(biweeklySalary)
...
export function calculatePayroll(input: PayrollInput): PayrollCalculation {
  ...
  const leaveCashoutAmount = calculateLeaveCashout(input.grossSalary, leaveCashoutDays)
  const debtPayoutAmount = input.debtPayoutAmount || 0

  // Total gross includes salary + leave cashout + debt payout
  // Note: Debt payout is typically NOT subject to TDS as it's paying back owed salary
  // TDS is only on regular salary + leave cashout
  const taxableAmount = input.grossSalary + leaveCashoutAmount
  const tdsAmount = calculateTds(taxableAmount, input.tdsRate)

  // Net = taxable amount - TDS + debt payout (debt is not taxed again)
 ...
 */
export function getNextPayrollDate(lastPayrollDate: Date): Date {
...
export function getOverduePayrollDates(lastPayrollDate: Date): Date[] {
  ...
  let nextDate = getNextPayrollDate(lastPayrollDate)

  while (nextDate <= today) {
    overdue.push(new Date(nextDate))
    nextDate = getNextPayrollDate(nextDate)
  }
 ...
 * Pay period is 14 days ending on run date
 */
export function getPayPeriod(runDate: Date): { start: Date; end: Date } {
  const end = new Date(runDate)
  end.setHours(23, 59, 59, 999)

  const start = new Date(runDate)
  start.setDate(start.getDate() - 13) // 14 day period (including run date)
